...
extern const uint32_t RollingCrc32Lookup[256];
#endif
...
{
  crc = ~crc;
  crc = (crc >> 8) ^ Crc32Lookup[0][(crc & 0xFF) ^ cur] ^ RollingCrc32Lookup[pre];
  return ~crc;
}
...
{
  crc = ~crc;
  crc = crc ^ RollingCrc32Lookup[pre];
  return ~crc;
}
...
#ifndef NO_LUT
// Rolling look-up table for CRC32
const uint32_t RollingCrc32Lookup[256] = {
