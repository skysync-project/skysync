...
extern const uint32_t RollingCrc32Lookup4KB[256];
extern const uint32_t RollingCrc32Lookup8KB[256];
#endif
...
{
  crc = ~crc;
  crc = (crc >> 8) ^ CrcTable[(crc & 0xFF) ^ cur] ^ RollingCrc32Lookup4KB[pre];
  return ~crc;
}
...
{
  crc = ~crc;
  crc = (crc >> 8) ^ CrcTable[(crc & 0xFF) ^ cur] ^ RollingCrc32Lookup8KB[pre];
  return ~crc;
}
...
{
  crc = ~crc;
  crc = crc ^ RollingCrc32Lookup4KB[pre];
  return ~crc;
}
...
uint32_t crc32_isal(const void* data, size_t length, uint32_t previousCrc32) {
  return crc32_gzip_refl(previousCrc32, data, length);
}
...
// Calculate CRC32(A'), where A' is A with lengthB zeros appended
...
// crc32_remove0 is the same as crc32_combine_return_crcA0, but removes the trailing zeros
uint32_t crc32_rem0(uint32_t crc1, size_t len2) {
    // remove the effect of the zeros added to crc1
    // This is the inverse operation of crc32_combine_return_crcA0
 ...
 //
    ...
    // Column 31 is 0xDB710641
    for (int i = 0; i < (int)CrcBits - 1; i++)
        odd[i] = 1U << (i + 1);
    odd[CrcBits - 1] = 0xDB710641;
...
#define PIGZ_BLOCK 131072UL
#define PIGZ_SHIFT x2nmodp(PIGZ_BLOCK, 3)
...
uint32_t crc32_comb(uint32_t crc1, uint32_t crc2, size_t len2) {
    return multmodp(len2 == PIGZ_BLOCK ? PIGZ_SHIFT : x2nmodp(len2, 3), crc1) ^ crc2;
}

uint32_t crc32_add0(uint32_t crc1, size_t len2) {
    return multmodp(len2 == PIGZ_BLOCK ? PIGZ_SHIFT : x2nmodp(len2, 3), crc1);
}

#define IDENTITY_OP ((uint32_t)1 << 31)
#define INV_BYTE_OP 0x38fb2284
uint32_t operator_pow(uint32_t base, size_t exp) {
    uint32_t result = IDENTITY_OP;
    while (exp > 0) {
        if (exp & 1) {
            result = multmodp(base, result);
        }
        base = multmodp(base, base);
        exp >>= 1;
    }
    return result;
}
...
uint32_t crc32_remove0(uint32_t crc1, size_t len2) {
    uint32_t inv_op = operator_pow(INV_BYTE_OP, len2);
    return multmodp(inv_op, crc1);
}
...
// #ifndef NO_LUT
// Rolling look-up table for CRC32 for 4KB Windows
...
const uint32_t RollingCrc32Lookup4KB[256] = {
...
const uint32_t RollingCrc32Lookup8KB[256] = {
    0xFD4CD5DD, 0xB3FD307F, 0x602F1E99, 0x2E9EFB3B, 0x1CFA4514, 0x524BA0B6, 0x81998E50, 0xCF286BF2,
    0xE550F20E, 0xABE117AC, 0x7833394A, 0x3682DCE8, 0x04E662C7, 0x4A578765, 0x9985A983, 0xD7344C21,
    0xCD749A7B, 0x83C57FD9, 0x5017513F, 0x1EA6B49D, 0x2CC20AB2, 0x6273EF10, 0xB1A1C1F6, 0xFF102454,
    0xD568BDA8, 0x9BD9580A, 0x480B76EC, 0x06BA934E, 0x34DE2D61, 0x7A6FC8C3, 0xA9BDE625, 0xE70C0387,
    0x9D3C4A91, 0xD38DAF33, 0x005F81D5, 0x4EEE6477, 0x7C8ADA58, 0x323B3FFA, 0xE1E9111C, 0xAF58F4BE,
    0x85206D42, 0xCB9188E0, 0x1843A606, 0x56F243A4, 0x6496FD8B, 0x2A271829, 0xF9F536CF, 0xB744D36D,
    0xAD040537, 0xE3B5E095, 0x3067CE73, 0x7ED62BD1, 0x4CB295FE, 0x0203705C, 0xD1D15EBA, 0x9F60BB18,
    0xB51822E4, 0xFBA9C746, 0x287BE9A0, 0x66CA0C02, 0x54AEB22D, 0x1A1F578F, 0xC9CD7969, 0x877C9CCB,
    0x3DADEB45, 0x731C0EE7, 0xA0CE2001, 0xEE7FC5A3, 0xDC1B7B8C, 0x92AA9E2E, 0x4178B0C8, 0x0FC9556A,
