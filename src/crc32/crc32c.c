...
uint32_t doCrc_rolling(uint32_t crc, const uint8_t cur, const uint8_t pre)
{
    crc = ~crc;
    crc = (crc >> 8) ^ crct[(crc ^ cur) & 0xff] ^ RollingTable[pre];
    return ~crc;
}
...
uint32_t crc32c_isal(uint32_t crc, const void *input, uint64_t len) {
    const char* buf = (const char*)input;
    crc ^= 0xFFFFFFFF;
    crc = crc32_iscsi((unsigned char *)buf, len, crc);
