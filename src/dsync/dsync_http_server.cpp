...
class DsyncHandler : public net::http::HTTPHandler {
public:
    int handle_request(net::http::Request& req, net::http::Response& resp, std::string_view) override {
        ...
        if (basis_filename.empty()) {
            ...
            error_resp.set_error_message("basis_filename parameter is required");
        ...
        if (path_str == "/csums_queue" && req.verb() == net::http::Verb::POST) {
            ...
            // Initialize ServerSyncWorker with appropriate algorithms based on hw parameter
            ...
            try {
                ...
                deserialize_uint32_queue_from_protobuf(pb_queue, server_worker.new_crc32_queue);
            } catch (const std::exception& e) {
                LOG_ERROR("Failed to process csums_queue protobuf: `", e.what());
            ...
            if (old_file_s == -1) {
                ...
                error_resp.set_error_message("Failed to get file size");
                error_resp.set_timestamp(std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::system_clock::now().time_since_epoch()).count());
            ...
            // server_worker.lhash_builder(server_worker.old_csums_queue, old_file_s / 8192 / 2);
            ...
            server_worker.compare_weak_chash(old_fd, server_worker.new_crc32_queue, server_worker.weak_matched_chunks_queue_1);
            diff_1 = std::chrono::high_resolution_clock::now() - start;
            LOG_INFO("Weak hash comparison completed in ` seconds", diff_1.count());

            // Serialize weak matched data using protobuf
            ...
            // Create combined protobuf response
            ...
            *response_pb.mutable_weak_matched_data() = weak_matched_pb;
            ...
            if (!response_pb.SerializeToString(&response_data)) {
                LOG_ERROR("Failed to serialize CsumsQueueResponse to protobuf");
                
                // Create protobuf error response
                dsync::ErrorResponse error_resp;
                error_resp.set_error_code(500);
                error_resp.set_error_message("Failed to serialize response");
                error_resp.set_timestamp(std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::system_clock::now().time_since_epoch()).count());
            ...
            // Create protobuf error response
            dsync::ErrorResponse error_resp;
            error_resp.set_error_code(405);
            error_resp.set_error_message("Method Not Allowed or Unsupported Path");
            error_resp.set_timestamp(std::chrono::duration_cast<std::chrono::seconds>(
                std::chrono::system_clock::now().time_since_epoch()).count());
...
int main(int argc, char** argv) {
    ...
    DsyncHandler handler;
    http_srv->add_handler(&handler);

    tcpserv->set_handler(http_srv->get_connection_handler());
    tcpserv->start_loop();

    LOG_INFO("Dsync HTTP Server started on port `", FLAGS_port);

    while (!stop_flag) {
        photon::thread_sleep(1);
    }
    LOG_INFO("Dsync HTTP Server stopped");
    return 0;
}