...
DEFINE_string(server_ip, "127.0.0.1", "SkySyncC server IP address");
DEFINE_uint64(server_port, 19876, "SkySyncC server port");
...
int perform_skysyncc_client_flow(const std::string& server_ip, uint64_t server_port,
                              const std::string& basis_filename, const std::string& new_filename,
                              bool hw) {
    LOG_INFO("Starting SkySyncC client flow...");
    ...
    // client_worker.lhash_builder(client_worker.new_csums_queue, client_worker.new_crc32_queue, new_file_s / 8192);
    client_worker.hash7_builder(client_worker.new_csums_queue, client_worker.new_crc32_queue, new_file_s / 8192);
    ...
    client_worker.compare_sha1_hash7(new_fd, client_worker.weak_matched_chunks_queue_1, client_worker.strong_matched_chunks_queue);
    ...
    LOG_INFO("SkySyncC client flow completed successfully.");
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    return perform_skysyncc_client_flow(FLAGS_server_ip, FLAGS_server_port,
