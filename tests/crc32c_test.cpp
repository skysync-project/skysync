...
//   char* data = new char[NumBytes];
  ...
  // rolling_crc32_1byte_4KB using a DefaultWindowSize window
  auto start_1 = std::chrono::high_resolution_clock::now();
  crc = crc32_fast(data, DefaultWindowSize , 0);
  for (uint64_t i = DefaultWindowSize; i < NumBytes; i++)
  {
    crc = rolling_crc32_1byte_4KB(crc, data[i], data[i - DefaultWindowSize]);
    if((crc & break_mask) == magic_number)
      continue;
  }
  std::chrono::duration<double> diff = std::chrono::high_resolution_clock::now() - start_1;
  printf("rolling crc32 1byte for 4KB windows  :  CRC32=%08X, %.3fs, %.3f MB/s\n",
          crc, diff.count(), (NumBytes / (1024*1024)) / diff.count());

  // rolling_crc32_1byte_4KB using a 8KB window
  start_1 = std::chrono::high_resolution_clock::now();
  uint32_t size_8KB = 8 * 1024;
  crc = crc32_fast(data, size_8KB , 0);
  for (uint64_t i = size_8KB; i < NumBytes; i++)
  {
    crc = rolling_crc32_1byte_8KB(crc, data[i], data[i - size_8KB]);
    if((crc & break_mask) == magic_number)
      continue;
  }
  diff = std::chrono::high_resolution_clock::now() - start_1;
  printf("rolling crc32 1byte for 8KB windows  :  CRC32=%08X, %.3fs, %.3f MB/s\n",
          crc, diff.count(), (NumBytes / (1024*1024)) / diff.count());
  ...
  // diff = std::chrono::high_resolution_clock::now() - start_1;
  // printf("rolling rsync       :  hash=%08X, %.3fs, %.3f MB/s\n",
  //       crc, diff.count(), (NumBytes / (1024*1024)) / diff.count());

  // SS-CDC rolling hash
  start_1 = std::chrono::high_resolution_clock::now();
  // uint32_t ss_crc = crc32c_hw(0, (const unsigned char*)data, DefaultWindowSize);
  uint32_t ss_crc = crc32_fast(data, DefaultWindowSize, 0);
  for (uint64_t i = 0; i + DefaultWindowSize < NumBytes; i++) {
