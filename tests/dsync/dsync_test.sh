...
# 14 datasets for each two files to be compared
datasets=(
...
do
    ...
    base_name=${base_name%.*}
    echo "Sync task test: $old_file $new_file"
    echo "$base_name,time" >> $log_file
    for j in $(seq 1 3); 
    # for j in 1;
    do
        sudo sync && sudo sysctl -w vm.drop_caches=3
        sudo ./sync_task_test $old_file $new_file >> $log_file
    done
done