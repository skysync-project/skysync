...
run_command() {
    ...
    if [ $enable_perf -eq 1 ]; then
        perf record -F1000 -g -o $dir_dst/digs_perf.data $srcdir/rsync_test --block-size=8192 --rollsum=rollsum --hash=md4 signature $1 $dir_dst/$base_name-digs-rsync
    else
        $srcdir/rsync_test --block-size=8192 --rollsum=rollsum --hash=md4 signature $1 $dir_dst/$base_name-digs-rsync
    fi
    ...
    if [ $enable_perf -eq 1 ]; then
        perf record -F1000 -g -o $dir_dst/delta_perf.data $srcdir/rsync_test --block-size=8192 --rollsum=rollsum --hash=md4 delta $dir_dst/$base_name-digs-rsync $2 $dir_dst/$base_name-delta-rsync
    else
        $srcdir/rsync_test --block-size=8192 --rollsum=rollsum --hash=md4 delta $dir_dst/$base_name-digs-rsync $2 $dir_dst/$base_name-delta-rsync
    fi
